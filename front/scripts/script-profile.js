...
window.onload = function() {
  ...
  let heroListDirectives = {
	...
	function getData() {
		let req = "/api/v2/player" + "/" + platform + "/" + nick + "/" + mode
		console.log(req);
		$.get(req, (data) => {
			...
			if (mode == "quickplayStats") {
				...
				let hasRank = (data.globalStats||{}).rank;
				if( hasRank ) {
			...
			let averagesObj = data[mode].global.average;
			
			let overallScore = calculatePercentiles(averagesObj, statsObj);
			if ( mode == "competitiveStats" ){
				title = "Competitive Overall Statistics"
			}else{
				title = "Quickplay Overall Statistics"
			}
			
			let heroList = [];
			
			function generateHeroList(heroesObj) {
				let curHero;
				Object.keys(heroesObj).forEach( (key) => {
					curHero = {};
					let hasCumulative = (heroesObj[key]||{}).cumulative;
					if (hasCumulative) {
						let gp = heroesObj[key].cumulative.games_won || 0;
						if (gp) {
							
							curHero.clickRow = "/player/" + platform + "/" + nick + "/" + key + "?mode=" + mode;
							curHero.heroName = key.replace(":_", "");
							curHero.ed = heroesObj[key].average.eliminations_per_life || 0;
							curHero.time = heroesObj[key].time.time_played || 0;
							
							
							heroList.push(curHero);
			...
			if ( mode == "competitiveStats" ) {
				generateHeroList( data.competitiveStats.heroes );
			} else {
				generateHeroList( data.quickplayStats.heroes );
			}
